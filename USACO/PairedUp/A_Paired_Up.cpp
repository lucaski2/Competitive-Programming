...
int solve(vector<pair<int, int>> a, int k)
{
  ...
  {
    dp[i] = dp[i + 1];
    pair<int, int> p = {a[i].f + k, 1e9};
    int ind = upper_bound(all(a), p) - a.begin();

    if (i == 0 or i == n - 1 or a[i + 1].f - a[i - 1].f <= k or (n - i & 1))
    {
      dp[i].s = max(dp[i].s, dp[ind].f + a[i].s);
    }
    if (i == 0 or i == n - 1 or a[i + 1].f - a[i - 1].f <= k or !(n - i & 1))
    {
      dp[i].f = max(dp[i].f, dp[ind].s + a[i].s);
    }
  }

  return (n & 1 ? dp[0].s : dp[0].f);
}
  ...
  if (t == 1)
  {
    ...
    for (auto cur : comp)
    {
      ...
      for (int i = 0; i < cur.size(); i++)
      {
        ...
        {
          if (cur[i - 1].f + k >= cur[i + 1].f) lo = min(lo, cur[i].s);
        }
      }
      ans += lo;
    }
