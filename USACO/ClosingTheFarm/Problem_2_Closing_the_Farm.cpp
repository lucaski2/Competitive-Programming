...
void solve(int tc)
{
  ...
  // to be correct, number of merges from different sets must be equal to i
  int merges = 0;
  vector<bool> awake(n);
  vector<string> ans(n);
  for (int i = 0; i < n; i++)
  {
    for (int edge : graph[a[i]])
    {
      if (awake[edge] and !dsu.sameSet(edge, a[i]))
      {
        merges++;
        dsu.unite(a[i], edge);
      }
  ...
  for (string asdf : ans) cout << asdf << endl;
}
