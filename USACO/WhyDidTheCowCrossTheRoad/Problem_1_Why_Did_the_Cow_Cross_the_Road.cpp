...
void solve(int tc)
{
  ...
  vector<vector<vector<int>>> dists(n, vector<vector<int>>(n, vector<int>(3, INT64_MAX)));
  ...
  while (!pq.empty())
  {
    ...
    if (dists[u.x][u.y][u.m] < u.d) continue;
    ...
    for (int i = -1; i <= 1; i++)
    {
      for (int j = -1; j <= 1; j++)
      {
        ...
        int next = u.d + t + (u.m == 1) * grid[x][y];
        if (next < dists[x][y][(u.m + 1) % 3])
        {
          pq.push({x, y, next, (u.m + 1) % 3});
          dists[x][y][(u.m + 1) % 3] = next;
        }
      }
    }
  }
  cout << *min_element(all(dists[n - 1][n - 1])) << endl;
