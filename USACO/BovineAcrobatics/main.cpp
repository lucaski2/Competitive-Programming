...
int main()
{
    ...
    cur.push_back({mint, m});

    int ptr = 0;
    
    while (ptr < n and cur.size() > 0)
    {
        vector<pair<ll, ll>> next;
        for (int i = 0; i < cur.size(); i++)
        {
            ll weight = cur[i].first;
            ll am = cur[i].second;
            while (ptr < n and am > 0)
            {
                if (weights[ptr].first > weight - k)
                {
                    ptr++;
                    continue;
                }
                ll take = min(am, weights[ptr].second);
                weights[ptr].second -= take;
                am -= take;
                ans += take;
                // next.push_back({weights[ptr].first, take});
                if (next.size() > 0 and next[next.size() - 1].first == weights[ptr].first)
                    next[next.size() - 1].second += take;
                else
                    next.push_back({weights[ptr].first, take});
                
                if (weights[ptr].second == 0)
                    ptr++;
            }
        }
