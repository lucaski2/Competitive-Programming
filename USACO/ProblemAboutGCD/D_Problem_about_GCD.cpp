...
void solve(int tc)
{
  ...
  // a and a + 1 are always coprime a >= 2
  if (g > r)
  {
    cout << -1 << ' ' << -1 << endl;
    return;
  ...
  pair<int, int> best = {lo * g, lo * g + g};
  if (lo * g + g > r) best = {lo * g, lo * g};
  for (int i = lo; i < min(hi, lo + 20); i++)
  {
    for (int j = hi; j >= max(lo, max(i + 1, hi - 20)); j--)
    {
      if (gcd(i, j) == 1)
      {
        if (g * j - g * i > best.s - best.f)
        {
          best = {g * i, g * j};
        }
      }
