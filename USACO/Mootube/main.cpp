...
int get_root(vector<int>& dsu, int a)
{
    while (dsu[a] >= 0) a = dsu[a];
    return a;
}

int merge(vector<int>& dsu, int a, int b)
{
    a = get_root(dsu, a);
    b = get_root(dsu, b);
    if (a == b) return 0;
...
int main()
{
    ...
    cin.tie(NULL);
    // open mootube.in and mootube.out
    ...
    for (int i = 0; i < n - 1; i++)
    {
        scanf("%d %d %d", &edges[i][2], &edges[i][1], &edges[i][0]);
        edges[i][2]--;
        edges[i][1]--;
    }
    ...
    {
        int am = queries[i][0];
        while (ptr >= 0 and edges[ptr][0] >= am)
        {
            merge(dsu, edges[ptr][1], edges[ptr][2]);
            ptr--;
        }

        ans[queries[i][2]] = -dsu[get_root(dsu, queries[i][1])] - 1;
    }
