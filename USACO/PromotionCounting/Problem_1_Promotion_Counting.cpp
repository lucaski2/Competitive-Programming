...
using ost_t = ost<pair<int,int>>;
using ost_ptr = unique_ptr<ost_t>;

ost_ptr dfs(int cur) {
    // allocate a fresh tree for node cur
    ost_ptr elements = make_unique<ost_t>();

    for (int nei : graph[cur]) {
        ost_ptr child = dfs(nei);
        if (elements->size() < child->size()) 
            swap(elements, child);    // just swaps the unique_ptrs
        for (auto &p : *child) 
            elements->insert(p);
    }

    ans[cur] = elements->size() 
             - elements->order_of_key({a[cur], -1});
    elements->insert({a[cur], cur});
    return elements;  // this moves the unique_ptr, no deep copy
}
