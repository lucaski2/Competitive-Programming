...
void solve(int tc)
{
  ...
  for (int i = 0; i < n - 1; i++)
  {
    ...
    cin >> a >> b >> c;
    a--; 
    graph[a].push_back({i + 1, b, c});
  }

  int dist = 0;

  function<void(int)> findDists = [&](int cur)
  {
    // no visited i am too smart
    dists[cur] = dist;
    for (info neighbor : graph[cur])
    {
      dist += neighbor.e;
      findDists(neighbor.a);
      dist -= neighbor.e;
    }
  };
  findDists(0);
  ...
  function<void(int, multiset<int>)> calcLarge = [&](int cur, multiset<int> large)
  {
    if (large.size() > mx) large.erase(large.begin());
    blocks[cur] = large;

    for (info neighbor : graph[cur])
    {
      large.insert(neighbor.d);
      calcLarge(cur, large);
      large.erase(large.find(neighbor.d));
    }
  };

  vector<vector<int>> blocks2(n)

  for (int i = 0; i < n; i++)
  {
    for (int ele : blocks[i])
    {
      blocks2[i].push_back(ele);
    }
  }

  vector<vector<pair<int, int>>> mins(mx);
