...
void solve(int tc)
{
  // first, find length of largest sorted suffix, add to ost
  // then, find index that it needs to be inserted into
  ...
  int ans = n - suff.size();
  cout << ans << endl;
  for (int i = 0; suff.size() < n; i++)
  {
    int am = suff.order_of_key(a[i]) + n - suff.size() - 1;
    cout << am;
    suff.insert(a[i]);
    if (suff.size() != n) cout << ' ';
