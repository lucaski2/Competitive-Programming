...
void solve(int tc)
{
  ...
  sparse_table<pair<int, int>> sparse(lca_et);
  ...
  while (q--)
  {
    ...
    if (t == 2)
    {
      ...
      cin >> x >> y;
      x--; y--;
      pair<int, int> lca = sparse.query(min(first[x], first[y]), max(first[x], first[y]));
      cout << ((seggy.query(early[x]) ^ seggy.query(early[y])) ^ a[lca.s]) << endl;
    }
  }
}
