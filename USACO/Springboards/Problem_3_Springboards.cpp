...
struct Seggy {
    ...
    void upd(int n, int l, int r) {
        if(l == r) { data[n] = max(data[n], val); return; }
        if(ql <= m) { upd(2*n, l, m); }
...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    // if (comp)
    // {
    //   comp2 = pq.top().x < t[i].sx;
    //   comp2 &= (pq.top().x == t[i].sx and pq.top().y < t[i].sy);
    // }
    while (pq.size() and (pq.top().x < t[i].sx or (pq.top().x == t[i].sx and pq.top().y < t[i].sy)))
    {
      info2 cur = pq.top();
      pq.pop();
      seggy.update(cur.y, cur.am);
    }
    int mx = seggy.query(0, t[i].sy);
    pq.push({t[i].ex, t[i].ey, mx + t[i].d});
    ans = max(ans, mx + t[i].d);
    // cout << mx << endl;
  }
