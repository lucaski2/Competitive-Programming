...
map<int, int> dfs(int cur, int par, int depth)
{
  map<int, int> depths;
  depths[depth] = 1;
  int incr = 0;
  ...
  for (int neighbor : graph[cur])
  {
    ...
    map<int, int> ret = dfs(neighbor, cur, depth + 1);

    if (ret.size() > depths.size()) swap(ret, depths);
    for (auto ele : ret)
    {
      int needed = depth + k - (ele.f - depth);
      if (depths.find(needed) != depths.end())
      { 
        int val = ele.s * depths[needed];
        ans += val;
        incr += val;
      }
    }
