...
int dfs(vector<vector<int>> &graph, vector<int> &values, int node, bool is_root)
{
    if (graph[node].empty())
    {
        return values[node];
    ...
    {
        am = min(am, dfs(graph, values, graph[node][i], false));
    }
    if (is_root)
    {
        return values[node] + am;
    }
    if (am <= values[node])
    {
        return am;
    }
    return (am + values[node]) / 2;
}
...
void solve()
{
    ...
    int ans = dfs(graph, values, 0, true);
