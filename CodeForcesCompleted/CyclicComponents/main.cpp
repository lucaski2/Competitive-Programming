...
void dfs(vector<vector<int>> &graph, int node, set<int> &not_visited, vector<int> &component)
{
    for (int i : graph[node])
    {
        if (not_visited.find(i) != not_visited.end())
        {
            ...
            dfs(graph, i, not_visited, component);
        }
    }
}
int main()
{
    ...
    int n, m;
    cin >> n >> m;
    vector<vector<int>> g = graph_edges_al(n, m);
    set<int> not_visited;
