...
void solve(int tc)
{
  int q;
  cin >> q;
  pair<int, int> range = {1, INT64_MAX};
  for (int i = 0; i < q; i++)
  {
    int type;
    cin >> type;
    ...
    if (type == 1)
    {
      int a, b, c;
      cin >> a >> b >> c;
      // means that range is between (a - b) * (c - 1) and (a - b) * (c - 1) + a
      int l = (a - b) * (c - 1) + b + 1;
      int r = (a - b) * (c - 1) + a;
      if (l > range.s or r < range.f)
      {
        cout << 0 << ' ';
        continue;
      }
      range.f = max(range.f, l);
      range.s = min(range.s, r);
      cout << 1 << ' ';
    }
    else
    {
