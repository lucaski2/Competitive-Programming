...
void solve(int tc)
{
  ...
  function<bool(double, double)> equals = [&](double a, double b)
  {
    return abs(a - b) < 1e-2;
  };
  ...
  for (int i = 0; i < n - 1; i++)
  {
    ...
    for (int j = i + 1; j < n; j++)
    { 
      prevs.push_back({abs(a[i].s - a[j].s) / abs(a[i].f - a[j].f), j});
    }
    sort(all(prevs), [](const info a, const info b) {
      return a.s < b.s;
    });

    for (int j = 0; j < prevs.size() - 1; j++)
    {
      if (equals(prevs[j].s, prevs[j + 1].s))
      {
        // get equation
        double m = prevs[j].s;
        // use i
        // a[i].s = m * a[i].f + b;
        double b = a[i].s - m * a[i].f;
        bool found = false;
        for (pair<double, double> cur : curs)
        {
          if (equals(cur.f, m) and equals(cur.s, b)) found = true;
        }
        if (!found)
        {
          curs.push_back({m, b});
