...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        max_time = max(max_time, p[i] - cur_track);
        cur_track += d[i];
    }

    ll ans = cur_track + max_time - d[n - 1];

    for (int i = n - 1; i > are 0; i--)
    {
        max_time = max(max_time - d[i], p[i]);
        ans = min(ans, max_time + cur_track - d[i - 1]);
    }
