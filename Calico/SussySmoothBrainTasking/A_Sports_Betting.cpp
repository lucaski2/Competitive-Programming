...
void solve(int tc)
{
  ...
  for (int i = 0; i < n * m; i++)
  {
    int col_time = min(abs(mp[i].f - mp[i + 1].f), min(mp[i].f, mp[i + 1].f) + n - max(mp[i].f, mp[i + 1].f));
    int row_time = min(abs(mp[i].s - mp[i + 1].s), min(mp[i].s, mp[i + 1].s) + m - max(mp[i].s, mp[i + 1].s));
    ans += col_time + row_time;
  }
  cout << ans << endl;
