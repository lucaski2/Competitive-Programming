...
void solve(int tc)
{
  ...
  for (auto ele : ms)
  {
    auto ptr = ms.upper_bound(ele);
    if (ptr != ms.end()) diffs.insert(ptr->f - ele.f);
  }
  ...
  while (q--)
  {
    ...
    ms.erase(cur);
    auto ptr1 = ms.upper_bound(cur);
    auto ptr2 = ms.lower_bound(cur);
    ...
    if (diff1 != -1) 
    {
      diffs.erase(diffs.find(diff1));
      // cout << 1 << endl;
    }
    if (diff2 != -1)
    {
      diffs.erase(diffs.find(diff2));
      // cout << 2 << endl;
    }
    if (diff1 != -1 and diff2 != -1)
    {
      // cout << 3 << endl;
      diffs.insert(diff1 + diff2);
    }

    cur = {x, i};

    ptr1 = ms.upper_bound(cur);
    ptr2 = ms.lower_bound(cur);
    diff1 = -1;
    diff2 = -1;
    ...
    {
      // cout << 6 << endl;
      // cout << "a " << diff1 << ' ' << diff2 << ' ' << diff1 + diff2 << endl;
      auto it = diffs.find(diff1 + diff2);
      // cout << diffs.count(*it) << endl;
      diffs.erase(it);
    }
    ...
    // cout << ms.rbegin()->f + *diffs.rbegin() << ' ' << ms.size() << ' ' << diffs.size() << endl;
    cout << ms.rbegin()->f + *diffs.rbegin() << ' ';
