...
void solve(int tc)
{
  ...
  // for each house calculate amount
  // [a[i], a[i + 1]) would be included
  ...
  for (int i = 0; i < n; i++)
  {
    // if you can make it the median
    int sz = n - k;
    int ind1 = i - (sz + 1) / 2;
    int ind2 = i + (sz + 1) / 2;
    int ind3 = i - (n + 1) / 2;
    int ind4 = i + (n + 1) / 2;
    if (ind1 <= k and ind1 >= 0) ans += (i < n - 1 ? a[i + 1] - a[i] : 1);
    else if (n - ind2 - 1 <= k and ind2 < n) ans += (i < n - 1 ? a[i + 1] - a[i] : 1);\
    else if (ind3 <= k and ind3 >= 0) ans += (i < n - 1 ? a[i + 1] - a[i] : 1);
    else if (n - ind4 - 1 <= k and ind4 < n) ans += (i < n - 1 ? a[i + 1] - a[i] : 1);\
  }
