...
void solve(int tc)
{
  ...
  vector<pair<int, int>> dp(n, {1e14, 1e14});
  ...
  while (!pq.empty())
  {
    ...
    if (dp[cur.u].f < cur.hi)
      continue;
    
    for (pair<int, int> neighbor : graph[cur.u])
    {
      info next = {neighbor.f, min(cur.lo, neighbor.s), max(cur.hi, neighbor.s)};
      if (dp[next.u].f > next.hi)
      {
        dp[next.u] = {next.hi, next.lo};
        pq.push(next);
      }
    }
  }

  vector<pair<int, int>> dp2(n, {1e14, 1e14});
  dp2[n - 1] = {0, 1e9};
  pq.push({n - 1, 0, 0});
  ...
  while (!pq.empty())
  {
    ...
    if (dp2[cur.u].f < cur.hi)
      continue;
    
    for (pair<int, int> neighbor : graph[cur.u])
    {
      info next = {neighbor.f, min(cur.lo, neighbor.s), max(cur.hi, neighbor.s)};
      if (dp2[next.u].f > next.hi)
      {
        dp2[next.u] = {next.hi, next.lo};
        pq.push(next);
      }
    }
  }
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    {
      // element x is the minimum
      ans = min(ans, max(max(dp[neighbor.f].f, dp2[i].f), neighbor.s) + neighbor.s);
    }
  }
  cout << ans << endl;
