...
void solve(int tc)
{
  ...
  // the number of paths is the number of mountains + 1
  // a mountain is defined as an index i such that a[i] > a[i + 1] and a[i] > a[i - 1]

  // count the number of mountains over all subsets

  int ans = pow(2, n, mod) - 1 - n;
  ...
  for (int i = 1; i < n - 1; i++)
  {
    for (int j = i + 1; j < n - 1; j++)
    {
      ...
      int am = (pow(2, i - best, mod) - 1) * pow(2, best, mod) % mod;
      am *= pow(2, n - best2 - 1, mod);
      am %= mod;

      am *= pow(2, best2 - j, mod) - 1;
      am %= mod;
      ans += am;
      ans %= mod;
    }
