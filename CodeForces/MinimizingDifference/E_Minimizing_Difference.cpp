...
void solve(int tc)
{
  ...
  while (mp.size() != 1)
  {
    // cout << mp.size() << endl;
    int num;
    if (mp[mp.begin()->f] < mp[mp.rbegin()->f])
    {
      num = mp.begin()->f;
      if (mp[num] > k)
      {
        cout << mp.rbegin()->f - num;
        return;
      }
      int cnt = mp[num];
      mp.erase(num);
      // check if can merge
      int am = (mp.begin()->f - num) * cnt;
      // cout << am << endl;
      if (am <= k)
      {
        mp[mp.begin()->f] += cnt;
    ...
    else
    {
      ...
      if (mp[num] > k)
      {
        cout << num - mp.begin()->f << endl;
        return;
      }
      int cnt = mp[num];
      mp.erase(num);

      int am = (-mp.rbegin()->f + num) * cnt;
      // cout << am << endl;
      if (am <= k)
      {
        mp[mp.rbegin()->f] += cnt;
