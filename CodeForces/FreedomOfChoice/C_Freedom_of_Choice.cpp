...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    tot_l += l;
    tot_r += r;
    // total sum and count for each element, and indices of each element
    ...
    for (int j = 0; j < k; j++)
    {
      tot_sum[i] += cur[j].s;
      a[i][cur[j].f] = cur[j].s;
      inds[cur[j].f].push_back(i);
    }
  }

  int ans = INT64_MAX;

  for (int i = tot_l; i <= tot_r; i++)
  {
    if (!inds.count(i))
    {
      cout << 0 << endl;
    ...
    sum_other = min(sum_other, i - sum_cur.f);
    int am = i - sum_other;
    int extra = am - sum_cur.f;

    int lo = 0;

    for (int ind : inds[i])
    {
      int d = tot_sum[ind] - a[ind][i];
      d = min(d, b[ind].s);
