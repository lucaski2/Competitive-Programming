...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    for (auto root : dsu.roots)
      merges.insert(root.f);
    for (int neighbor : graph[i])
    {
      if (neighbor > i) continue;
      dsu.roots[dsu.get(neighbor)]++;
      if (dsu.roots[dsu.get(neighbor)] == dsu.size(neighbor)) merges.erase(dsu.get(neighbor));
    }
    ...
    dsu.roots[dsu.get(i)] = 0;
  }

  cout << dsu.roots.size() - 1 << endl;
