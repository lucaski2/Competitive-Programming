...
void solve(int tc)
{
  ...
  int l = *max_element(all(a)), r = n * *max_element(all(a));
  int ans = n * *max_element(all(a));
  while (l <= r)
  {
    int m = (l + r) / 2;

    Seggy<int> seggy;
    seggy.init(n, n * *max_element(all(a)));
    seggy.update(0, 0);
    ...
    for (int i = 1; i < n; i++)
    {
      ...
      // if (tc == 1 and m == 7 and i == 6)
      // {
      //   cout << best << endl;
      // }
      best = max(best - 1, 0ll);
      int am = seggy.query(best, i - 1) + a[i];
      seggy.update(i, am);
    }
    // if (tc == 1 and m == 7)
    // {
    //   cout << seggy.query(n - 1, n - 1) << endl;
    // }
    if (seggy.query(n - 1, n - 1) > m)
    {
      l = m + 1;
    }
    else
