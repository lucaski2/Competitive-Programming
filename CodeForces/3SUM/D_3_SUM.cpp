...
void solve(int tc)
{
  ...
  for (int i = 0; i < 10; i++)
  {
    for (int j = i; j < 10; j++)
    {
      for (int k = j; k < 10; k++)
      {
        if (i == j and j == k and diddysnumbers[j] < 3) continue;
        else if (i == j and (diddysnumbers[j] < 2 or !diddysnumbers[k])) continue;
        else if (j == k and (diddysnumbers[j] < 2 or !diddysnumbers[i])) continue;
        else if (!diddysnumbers[j] or !diddysnumbers[i] or !diddysnumbers[k]) continue;
        if ((i + j + k) % 10 == 3)
        {
          cout << "YES" << endl;
          return;
