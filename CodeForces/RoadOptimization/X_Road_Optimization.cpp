...
void solve(int tc)
{
  ...
  // dp[j][k], j is current speed and k is current k
  ...
  for (int i = 0; i < n; i++)
  {
    vector<vector<int>> next(n, vector<int>(k + 1, INT_MAX));
    for (int j = 0; j < n; j++)
    {
      for (int l = 0; l <= k; l++)
      {
        ...
        {
          // push to next[j][l + 1]
          next[j][l + 1] = min(next[j][l + 1], dp[j][l] + a[j].s * (a[i + 1].f - a[i].f));
        }
        // push to next[i][l]
        next[i][l] = min(next[i][l], dp[j][l] + a[i].s * (a[i + 1].f - a[i].f));
      }
    }
    dp = next;
  }
