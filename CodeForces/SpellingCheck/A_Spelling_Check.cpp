...
void solve(int tc)
{
  ...
  int run = 0;
  for (int i = 0; i < n - 1; i++)
    run = (run + (t[i] - 'a') * pow(mod2, i, mod)) % mod;
  int run2 = 0;
  for (int i = 0; i < n; i++)
  {
    run2 = (run2 + (s[i] - 'a') * pow(mod2, i, mod)) % mod;
    hash[i + 1] = run2;
  }
  ...
  for (int i = 0; i < n; i++)
  {
    int am = (hash[n] - hash[i + 1]) * pow(mod2, mod - 2, mod) % mod + mod * 1e4 + hash[i];
    am %= mod;
    if (am == run) ans.push_back(i); 
  }
  cout << ans.size() << endl;
  for (int ele : ans)
    cout << ele + 1 << ' ';
