...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    if (a[i] == last and a[i] == last2) continue;
    else if (a[i] == last)
    {
      last2 = a[i];
      ans++;
    ...
    {
      auto ptr = upper_bound(all(inds[last]), i); 
      auto ptr2 = upper_bound(all(inds[last2]), i);
      if (ptr == inds[last].end())
      {
        last2 = a[i];
        ans++;
      }
      else if (ptr2 == inds[last2].end())
      {
        last = a[i];
        ans++;
      }
