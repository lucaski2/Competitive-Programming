...
void solve(int tc)
{
  ...
  {
    vector<vector<vector<int>>> dp(n + 1, vector<vector<int>>(k, vector<int>(n, 1e12)));
    dp[0][0][0] = 0;
    ...
    for (int j = 0; j < n; j++)
    {
      for (int l = 0; l < k; l++)
      {
        for (int m = 0; m < n; m++)
        {
          if (dp[j][l][m] == 1e12) continue;
          dp[j + 1][l][m] = dp[j][l][m] + (j - m) * a[j];
        }
      }
      ...
      for (int l = 0; l < k - 1; l++)
      {
        ...
        {
          if (dp[j][l][m] == 1e12) continue;
          dp[j + 1][l + 1][j] = min(dp[j + 1][l + 1][j], dp[j][l][m]);
        }
      }
    }
    ans = min(ans, *min_element(all(dp[n][k - 1])));
    int ele = a[0];
    a.erase(a.begin());
    a.push_back(ele);
  }
