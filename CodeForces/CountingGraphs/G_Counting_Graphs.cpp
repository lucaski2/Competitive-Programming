...
/*
you can connect vertices x and y with weight z vertex
if both x and y's parent's edge has a lower weight
*/
...
void solve(int tc)
{
  ...
  sort(all(a), [](edge a, edge b) {return a.w < b.w; });
  int ans = 1;
  ...
  { 
    a[i].a--;
    a[i].b--;
    int am1 = dsu.size(a[i].a);
    int am2 = dsu.size(a[i].b);
    dsu.unite(a[i].a, a[i].b);
    if (am1 == 1 and am2 == 1) continue;
    int pairs = (am1 * am2 - 1);
    int am = pow((s - a[i].w + 1) % mod, pairs, mod);
    // cout << am << endl;  
    ans *= am;
    ans %= mod;
  }
