...
void solve(int tc)
{
  ...
  vector<vector<int>> dp(n, vector<int>(2, INT64_MAX));
  dp[0][0] = 0;
  dp[0][1] = ca[0];
  ...
  for (int i = 1; i < n; i++)
  {
    for (int j = 0; j <= 1; j++)
    {
      for (int k = 0; k <= 1; k++)
      {
        // j is last, k is current
        if (dp[i - 1][j] >= INT64_MAX) continue;
        ...
        bool ok = true;
        for (int l = 0; l < n; l++)
          if (last[l] == cur[l]) ok = false;
        if (ok)
        {
          dp[i][k] = min(dp[i][k], dp[i - 1][j] + ca[i] * k);
        }
      }
    }
  }
  ...
  vector<vector<int>> dp2(n, vector<int>(2, INT64_MAX));
  dp2[0][0] = 0;
  dp2[0][1] = cb[0];
  ...
  for (int i = 1; i < n; i++)
  {
    for (int j = 0; j <= 1; j++)
    {
      ...
      {
        // j is last, k is current
        if (dp2[i - 1][j] >= INT64_MAX) continue;
        vector<int> last(n);
        vector<int> cur(n);
        ...
        if (ok)
        {
          dp2[i][k] = min(dp2[i][k], dp2[i - 1][j] + cb[i] * k);
        }
      }
    }
  }
  if (min(dp2[n - 1][0], dp2[n - 1][1]) == INT64_MAX)
  {
    cout << -1 << endl;
    return;
  }
  cout << ans + min(dp2[n - 1][0], dp2[n - 1][1]) << endl;
