...
void solve(int tc)
{
  ...
  set<int> rem;
  if (d[0] < a[1]) rem.insert(0);
  if (d[n - 1] < a[n - 2]) rem.insert(n - 1);

  for (int i = 1; i < n - 1; i++)
    if (a[i - 1] + a[i + 1] > d[i]) rem.insert(i);
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    {
      s.erase(ele);
      auto ptr = s.upper_bound(ele);
      if (ptr != s.end()) marked.insert(*ptr);
      if (ptr != s.begin())
      {
        ptr--;
        marked.insert(*ptr);
    ...
    for (int ele : marked)
    {
      ...
      int am = 0;
      auto ptr = s.upper_bound(ele);
      if (ptr != s.end()) am += a[*ptr];
      ptr = s.lower_bound(ele);
      if (ptr != s.begin())
      {
        ptr--;
        am += a[*ptr];
      }
      if (am > d[ele]) rem.insert(ele);
    }
    marked.clear();
  }
