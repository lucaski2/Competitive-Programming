...
int inv(int a) { return pow(a, mod - 2, mod); }
...
void solve(int tc)
{
  ...
  vector<int> dp(m + 1);
  dp[0] = 1;
  sort(all(a), [](info a, info b) { return a.l < b.l; });
  vector<int> prob(m, 1);
  for (int i = 0; i < n; i++)
  {
    prob[a[i].r] *= (a[i].q - a[i].p) * inv(a[i].q) % mod;
    prob[a[i].r] %= mod;
  }
  ...
  for (int i = 0; i < n; i++)
  {
    int am = prob[a[i].r] * inv((a[i].l > 0 ? prob[a[i].l - 1] : 1)) % mod;
    am *= inv(a[i].q - a[i].p) * a[i].q % mod;
    am %= mod;

    am *= a[i].q * inv(a[i].q) % mod;
    am %= mod;
