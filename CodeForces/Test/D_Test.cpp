...
bool is_substr(string s, string t)
{
  ...
  for (int i = t.size(); i < s.size(); i++)
  {
    ...
    hash2 += 1e4 * mod;
    hash2 %= mod;
    hash2 *= pow(base, mod - 2, mod);
    hash2 += s[i] * pow(base, t.size() - 1, mod);
...
void solve(int tc)
{
  ...
  do
  {
    if (is_substr(a[0], a[1]) and is_substr(a[0], a[2])) 
    {
      ans = min(ans, a[0].size());
      continue;
    }
    if (is_substr(a[1], a[0]) and is_substr(a[1], a[2])) 
    {
      ans = min(ans, a[1].size());
      continue;
    }
    
    if (is_substr(a[2], a[0]) and is_substr(a[2], a[1])) 
    {
      ans = min(ans, a[2].size());
      continue;
    }
