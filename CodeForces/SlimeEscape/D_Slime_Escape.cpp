...
void solve(int tc)
{
  ...
  // either go to closest net positive on a side
  // or go to end
  
  // to find closest max seggy walk
  // to find min in range rmq
  // or just 2p
  k--;
  int ptr1 = k - 1;
  int ptr2 = k + 1;
  ...
  while (true)
  {
    ...
    // cout << h << ' ' << l1 << ' ' << l2 << endl;
    if (ptr1 < 0 and h + l1 >= 0)
    {
      cout << "YES" << endl;
      return;
    }
    if (ptr2 >= n and h + l2 >= 0)
    {
      cout << "YES" << endl;
      return;
    }
