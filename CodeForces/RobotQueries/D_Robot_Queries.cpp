...
void solve(int tc)
{
  ...
  for (int i = 0; i < q; i++)
  {
    info c;
    cin >> c.x >> c.y >> c.l >> c.r;
    pair<int, int> p = {pos[c.l - 1].f + pos[c.r].f, pos[c.l - 1].s + pos[c.r].s};
    p.f -= c.x;
    p.s -= c.y;
    pair<int, int> t = {c.x, c.y};
    if (mp.find(t) != mp.end())
    {
      if (*mp[t].begin() < c.l or *mp[t].rbegin() >= c.r) 
      {
        cout << "YES" << endl;
        continue;
      }
    ...
    if (mp.find(p) != mp.end())
    {
      auto ptr = lower_bound(all(mp[p]), c.l);
      if (ptr != mp[p].end() and *ptr < c.r) 
      {
        cout << "YES" << endl;
        continue;
      }
