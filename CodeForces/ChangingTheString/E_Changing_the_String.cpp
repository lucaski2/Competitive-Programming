...
void solve(int tc)
{
  // b->a
  // c->a
  // b->c c->a combo, essentially b->a
  // c->b b->a combo, essentially c->a
  // when using b->a, use earliest
  // when using c->a, use earliest
  ...
  for (int i = 0; i < q; i++)
  {
    if (queries[i] == make_pair('b', 'a')) ba.insert(i);
    else if (queries[i] == make_pair('c', 'a')) ca.insert(i);
    else if (queries[i] == make_pair('b', 'c')) bc.insert(i);
    else if (queries[i] == make_pair('c', 'b')) cb.insert(i);
  }
  ...
  {
    if (s[i] == 'a') continue;
    if (s[i] == 'c' and ca.size())
    {
      s[i] = 'a';
      ca.erase(*ca.begin());
      continue;
    }
    if (s[i] == 'b' and ba.size())
    {
      s[i] = 'a';
      ba.erase(*ba.begin());
      continue;
    ...
    if (s[i] == 'b')
    {
      // only thing you can do is the combo
      if (bc.empty() or ca.empty()) continue;
      if (*bc.begin() > *ca.rbegin()) continue;
      int ind = *bc.begin();
      bc.erase(ind);
      ca.erase(*ca.upper_bound(ind));
      s[i] = 'a';
      continue;
    }
    // check if none of them work
    if (cb.empty()) continue;
    if (ba.empty() or *cb.begin() > *ba.rbegin()) 
    {
      cb.erase(*cb.begin());
      s[i] = 'b';
      continue;
