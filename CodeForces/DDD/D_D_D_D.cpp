...
void solve(int tc)
{
  ...
  int hi_even = accumulate(all(even), 0ll) + accumulate(all(odd), 0ll) - (odd.size() % 2 == 0 ? 0 : odd[0]);
  int hi_odd = -1;
  if (odd.size() > 0)
  {
    hi_odd = accumulate(all(even), 0ll) + accumulate(all(odd), 0ll) - (odd.size() % 2 == 1 ? 0 : odd[0]);
  }
  ...
  for (int i = 0; i < n; i++)
  {
    if (dist[i][0] != -1 and dist[i][0] <= hi_even)
    {
      ans[i] = '1';
    }
    if (dist[i][1] != -1 and dist[i][1] <= hi_odd)
      ans[i] = '1';
  }
  cout << ans << endl;
