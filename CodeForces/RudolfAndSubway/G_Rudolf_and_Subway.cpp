...
void solve(int t)
{
  ...
  // the railway sets
  vector<vector<int>> sts(m);
  // the sets element i belongs to
  vector<vector<int>> toele(n);
  ...
  for (int i = 0; i < m; i++)
  {
    ...
    edges[i].a--;
    edges[i].b--;
    if (!tc.count(edges[i].c))
    {
      tc[edges[i].c] = cnt++;
    }
 
    sts[tc[edges[i].c]].push_back(edges[i].a);
    sts[tc[edges[i].c]].push_back(edges[i].b);
    toele[edges[i].a].push_back(tc[edges[i].c]);
    toele[edges[i].b].push_back(tc[edges[i].c]);
  }
  ...
  while (q.size())
  {
    ...
    for (int ele : sts[cur])
    {
      ...
      visited[ele] = true;
      for (int otro : toele[ele])
      {
        if (dist[otro] == INT_MAX)
        {
          q.push(otro);
          dist[otro] = dist[cur] + 1;
        }
      }
    }
  }
  ...
  int ans = INT_MAX;
  for (int ele : toele[end])
    ans = min(ans, dist[ele]);
  cout << ans << endl;
