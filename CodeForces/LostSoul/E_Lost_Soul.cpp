...
void solve(int tc)
{
  ...
  reverse(all(a));
  reverse(all(b));
  // strategy without removals
  // in the snake, for each element, find the earliest two, the maximum of those is the answer
  // do similar with removals

  vector<int> pos1(n, INT_MAX);
  vector<int> pos2(n, INT_MAX);
  int ans = 0;
  for (int i = n - 1; i >= 0; i--)
  {
    if (i % 2 == 0)
  ...
  for (int i = 0; i < n; i++)
  {
    ans = max(ans, n - max(pos1[i], pos2[i]));
  }
  ...
  for (int i = 0; i < n; i++)
  {
    if (i % 2 == 0)
    {
      if (prevs[a[i]].size())
      {
        if (i - prevs[a[i]].back() > 1)
        {
          ans = max(ans, n - i);
        }
        else if (i - prevs[a[i]][0] > 1)
        {
          ans = max(ans, n - i);
        }
      }
      prevs[a[i]].push_back(i);
    }
    else
    {
      if (prevs[b[i]].size())
      {
        if (i - prevs[b[i]].back() > 1)
        {
          ans = max(ans, n - i);
        }
        else if (i - prevs[b[i]][0] > 1)
        {
          ans = max(ans, n - i);
        }
      }
      prevs[b[i]].push_back(i);
    }
  }



  vector<vector<int>> prevs2(n);
  for (int i = 0; i < n; i++)
  {
    if (i % 2 == 0)
    {
      if (prevs2[b[i]].size())
      {
        if (i - prevs2[b[i]].back() > 1)
        {
          ans = max(ans, n - i);
        }
        else if (i - prevs2[b[i]][0] > 1)
        {
          ans = max(ans, n - i);
        }
      }
      prevs2[b[i]].push_back(i);
    }
    else
    {
      if (prevs2[a[i]].size())
      {
        if (i - prevs2[a[i]].back() > 1)
        {
          ans = max(ans, n - i);
        }
        else if (i - prevs2[a[i]][0] > 1)
        {
          ans = max(ans, n - i);
        }
      }
      prevs2[a[i]].push_back(i);
    }
  }
  cout << ans << endl;
