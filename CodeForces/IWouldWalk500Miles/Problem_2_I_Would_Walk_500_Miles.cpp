...
void solve(int tc)
{
  ...
  int a = 2019201913, b = 2019201949, m = 2019201997;

  vector<info> edges(n * (n - 1) / 2);
  int cnt = 0;
  for (int sz = n + n; sz >= 1; sz--)
  {
    ...
    while (i < j)
    {
      edges[cnt++] = {i - 1, j - 1, m - (m - a) * i - (m - b) * j};
      i++; j--;
    }
  }
  // for (auto ele : edges)
  //   cout << ele.w << ' ' << ele.u << ' ' << ele.v << endl;
  // cout << endl;
  cnt = n;
  DSU dsu;
  dsu.init(n);
