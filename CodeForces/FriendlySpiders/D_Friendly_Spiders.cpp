...
void solve(int tc)
{
  ...
  cin >> start >> end;
  start--; end--;
  vector<set<int>> graph(mx, set<int>());

  for (int i = 0; i < n; i++)
  { 
    for (int factor : factors[a[i]])
    {
      graph[factor].insert(i);
    }
  }
  if (start == end)
  {
  ...
  for (int factor : factors[a[start]])
  {
    graph[factor].erase(start);
  }
  ...
  while (!q.empty())
  {
    ...
    for (int fact : factors[a[cur]])
    {
      vector<int> neighbors;
      for (int neighbor : graph[fact]) neighbors.push_back(neighbor);
      for (int neighbor : neighbors)
      {
        // we remove if visited so it shouldn't be visited
        if (depth[neighbor] >= 0) continue;
        q.push(neighbor);

        // remove all instances
        for (int rfact : factors[a[neighbor]])
        {
          graph[rfact].erase(neighbor);
        }
  ...
  if (depth[end] == -1) cout << -1 << endl;
  if (depth[end] != -1)
  {
    cout << depth[end] + 1 << endl;
    int cur = end;
    vector<int> path;
    while (cur != -1)
    {
