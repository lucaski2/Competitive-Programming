...
void solve(int tc)
{
  ...
  // if first is > cur, then second is required
  // if first is <= cur, then second is optional
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    int mx;
    if (req.empty()) mx = INT_MIN;
    else mx = req.rbegin()->f;

    ans = min(ans, abs(mx - a[i].f));

    if (mx < a[i].f)
    {
      if (!opt.empty())
      {
        auto ptr = opt.lower_bound({a[i].f, -1});
        if (ptr != opt.begin())
        {
          ptr--;
          ans = min(ans, a[i].f - ptr->f);
        }
        ptr = opt.lower_bound({a[i].f, -1});
        if (ptr != opt.end())
        {
          ans = min(ans, ptr->f - a[i].f);
        }
      }
    }
