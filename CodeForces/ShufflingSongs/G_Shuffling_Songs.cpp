...
int geth(string s)
{
  ...
  for (int i = 0; i < s.size(); i++)
    acc = (acc + pow(26, i, mod) * (s[i] - 26)) % mod;
  return acc;
}
...
void solve(int tc)
{
  ...
  int mx = (1 << n);
  vector<vector<vector<bool>>> dp(mx, vector<vector<bool>>(n, vector<bool>(n)));
  for (int i = 0; i < n; i++)
    dp[(1 << i)][i][i] = true;
  ...
  for (int mask = 0; mask < mx; mask++)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      for (int j = 0; j < n; j++)
      {
        ...
        for (int k = 0; k < n; k++)
        {
          ...
          int nmask = mask + (1 << k);
          if (a[i].s == a[k].s or a[i].f == a[k].f)
          {
            dp[nmask][k][j] = true;
          }
          if (a[j].s == a[k].s or a[j].f == a[k].f)
          {
            dp[nmask][i][k] = true;
          }
        }
    ...
    ans = max(ans, (int)__builtin_popcount(mask) * ok);
  }
