...
void solve(int tc)
{
  ...
  // we can group each into groups and pair diagonally
  set<int> left, right, lo, hi;
  sort(all(a), [](info a, info b) { return a.x < b.x; });
  ...
  sort(all(a), [](info a, info b) { return a.y < b.y; });
  for (int i = 0; i < n / 2; i++)
  {
    lo.insert(a[i].i);
  }
  ...
  for (int i = 0; i < n; i++)
  {
    if (left.count(i) and lo.count(i)) bl.insert(i);
    else if (left.count(i) and hi.count(i)) tl.insert(i);
    else if (right.count(i) and lo.count(i)) br.insert(i);
    else tr.insert(i);
  }

  // cout << tl.size() << ' ' << tr.size() << ' ' << bl.size() << ' ' << br.size() << endl;
  for (int ele : tl)
  {
    cout << ele + 1 << ' ' << *br.begin() + 1 << en;
    br.erase(*br.begin());
  }
  for (int ele : tr)
  {
    cout << ele + 1 << ' ' << *bl.begin() + 1 << en;
    bl.erase(*bl.begin());
  }
