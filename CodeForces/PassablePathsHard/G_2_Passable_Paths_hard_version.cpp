...
void solve(int tc)
{
  ...
  function<void(int, int, int)> dfs = [&](int cur, int depth, int par)
  {
    depths[cur] = depth;
    start[cur] = time;
    ...
    for (int neighbor : graph[cur])
    {
      ...
      dfs(neighbor, depth + 1, cur);
      ends[cur] = time;
      seggy.update(time++, {depth, cur});
    }
    if (ends[cur] == 0) ends[cur] = time;
  };
  dfs(0, 0, -1);
 
  function<pair<int, int>(int, int)> lca = [&](int a, int b)
  {
    return seggy.query(min(start[a], start[b]), max(start[a], start[b]));
  };
  ...
  while (q--)
  {
    ...
    for (int i = 1; i < c; i++)
    {
      if (start[a[i].s] < start[a[0].s] and ends[a[i].s] > start[a[0].s]) continue;
      ind = i;
      break;
    }
    if (ind == -1)
    ...
    int lca_depth = lca(a[ind].s, a[0].s).f;
    // cout << lca_depth << endl;
    bool ok = true;
    for (int i = 0; i < c; i++)
    {
      if (start[a[i].s] <= start[a[0].s] and ends[a[i].s] >= start[a[0].s] and depths[a[i].s] >= lca_depth) continue;
      if (start[a[i].s] <= start[a[ind].s] and ends[a[i].s] >= start[a[ind].s] and depths[a[i].s] >= lca_depth) continue;
      // cout << a[i].s << endl;
      // cout << a[ind].s << endl;
      // cout << a[0].s << endl;
      ok = false;
