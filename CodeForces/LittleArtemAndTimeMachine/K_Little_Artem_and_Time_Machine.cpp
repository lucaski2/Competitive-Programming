...
void solve(int tc)
{
  ...
  // change to query per x coord

  vector<vector<query>> query_number(n);
  for (int i = 0; i < n; i++)
  {
    query_number[mp[queries[i].x]].push_back(queries[i]); 
  }
  

  // compress times per x coord

  vector<vector<int>> times(n);

  for (int i = 0; i < n; i++)
  {
    times[mp[queries[i].x]].push_back(queries[i].t);
  }
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    Seggy<int> seggy;
    seggy.init(inds[i].size(), 0);
    for (query q : query_number[i])
    {
      if (q.a == 1)
      {
        seggy.update(inds[i][q.t], 1);
      }
      else if (q.a == 2)
      {
        seggy.update(inds[i][q.t], -1);
      ...
      {
        ans[q.i] = seggy.query(0, inds[i][q.t]);
      }
    }
  }
  for (int i = 0; i < n; i++)
