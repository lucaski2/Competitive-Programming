...
void solve(int tc)
{
  ...
  function<int(int, int)> dfs = [&](int cur, int am)
  {
    // try giving each child one of the values
    // choose the ones with the least difference
    if (cache.count({cur, am})) return cache[{cur, am}];
    int s = am * a[cur];
    vector<cc> info;
    info.reserve(graph[cur].size());
    for (int neighbor : graph[cur])
    {
      int n1 = dfs(neighbor, am / graph[cur].size());
      int n2 = dfs(neighbor, (am + graph[cur].size() - 1) / graph[cur].size());
      info.push_back({n1, n2, n2 - n1});
    }

    sort(all(info), [](cc c, cc d) { return c.dih > d.dih; });
    for (int i = 0; i < info.size(); i++)
    {
      int num_highs = am % graph[cur].size();
      if (i < num_highs)
      {
        s += info[i].hi;
      }
