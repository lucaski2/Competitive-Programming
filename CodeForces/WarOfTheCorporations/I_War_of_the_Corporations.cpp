...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    psum[i + 1] = (psum[i] + pow(base, i, mod) * (s[i] - 'a')) % mod;
  }

  int ans = 0;
  for (int i = 0; i < n - m + 1; i++)
  {
    int am = psum[i + m] - psum[i] + 1e5 * mod;
    am %= mod;
    am *= pow(pow(base, i, mod), mod - 2, mod);
    am %= mod;
    if (hash == am)
    {
      ans++;
