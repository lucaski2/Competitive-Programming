...
void solve(int tc)
{
  ...
  function<void(int)> dfs2 = [&](int cur)
  {
    ...
    if (!left and !right and (tree[cur].size() > 2 or (cur == 0 and tree[cur].size() >= 2)))
      left = true;
  ...
  if (dl == dr)
  {
    cout << pow(2, top + 1, mod) << endl;
  }
  


  else cout << (pow(2, top, mod) * pow(2, dr - dl, mod) + pow(2, top, mod) * pow(2, dr - dl - 1, mod)) % mod << endl;
