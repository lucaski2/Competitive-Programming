...
void solve(int tc)
{
  ...
  vector<vector<int>> psum(k, vector<int>(n + 1));
  for (int i = 0; i < k; i++)
  {
    for (int j = 0; j < n; j++)
    {
      psum[i][j + 1] = psum[i][j] + dist[a[j]][i];
    }
  }
  ...
  for (int i = d; i < n; i++)
  {
    for (int j = 0; j < k; j++)
    {
      dp[i + 1][j] = min(dp[i + 1][j], dp[i][j] + dist[a[i]][j]);
      // find something to stack onto j
      for (int l = 0; l < k; l++)
      {
        if (d + i > n) continue;
        int am = dp[i][j] + psum[l][d + i] - psum[l][i];
        dp[i + d][l] = min(dp[i + d][l], am);
      }
    }
  }
