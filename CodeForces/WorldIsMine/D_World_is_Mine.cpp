...
void solve(int tc)
{
  ...
  // for each index and number of ops used, minimize the amount of cakes
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    for (int j = 0; j <= n; j++)
    {
      ...
      // you can try not blocking meaning alice would get it
      dp[i + 1][j] = min(dp[i + 1][j], dp[i][j] + 1);
      // at this point alice has eaten dp[i][j] cakes
      if (dp[i][j] >= a[i] + j) dp[i + 1][a[i] + j] = min(dp[i + 1][a[i] + j], dp[i][j]);
    }
  }
