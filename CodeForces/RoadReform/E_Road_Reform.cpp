...
void solve(int tc)
{
  ...
  sort(all(a), [](info a, info b) { return a.w < b.w; });

  // add in guys <= k, theyre free
  DSU dsu;
  dsu.init(n);
  int ans = INT64_MAX;
  for (int i = 0; i < m; i++)
  {
    ...
    if (dsu.size(0) == n) ans = min(ans, k - a[i].w);
  }
  if (ans != INT64_MAX)
  {
    for (int i = 0; i < m; i++)
      ans = min(ans, abs(k - a[i].w));
  }
  else 
  {
    int s = 0;
    ...
    {
      if (a[i].w <= k) continue;
      if (!dsu.sameSet(a[i].a, a[i].b))
      {
        dsu.unite(a[i].a, a[i].b);
        s += a[i].w - k;
      }
      if (s > 0 and dsu.size(0) == n) ans = min(ans, s);
    }
  }
  cout << ans << endl;
