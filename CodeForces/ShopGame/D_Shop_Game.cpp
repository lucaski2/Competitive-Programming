...
// if k = 0, then just take every single number where a < b
// bob will always take the most expensive k guys
...
void solve(int tc)
{
  ...
  sort(all(a), [](pair<int, int> c, pair<int, int> d) { return c.s > d.s; });
  ...
  for (int i = 0; i < n; i++)
  {
    psum[i + 1] = psum[i] + max(0ll, a[i].s - a[i].f);
  }
  int ans = 0;
  for (int i = k; i < n; i++)
  {
    int am = psum[n] - psum[i] - s;
    ans = max(am, ans);
    s += a[i].f;
    cur.insert(a[i].f);
    s -= *cur.rbegin();
