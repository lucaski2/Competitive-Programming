...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    x.insert({a[i].f, a[i].s});
    y.insert({a[i].s, a[i].f});
  }

  // edge case is if theyre all in a rectangle with one outlier
  ...
  for (int i = 0; i < n; i++)
  {
    x.erase(x.find({a[i].f, a[i].s}));
    y.erase(y.find({a[i].s, a[i].f}));

    int width = (*x.rbegin()).f - (*x.begin()).f;
    int height = (*y.rbegin()).f - (*y.begin()).f;
    width++; height++;
    if (width * height == n - 1)
    {
      ans = min(ans, width * height + min(width, height));
    }
    else ans = min(ans, width * height);
