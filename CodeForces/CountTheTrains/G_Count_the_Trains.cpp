...
void solve(int tc)
{
  ...
  while (q--)
  {
    ...
    {
      ptr--;
      if (a[*ptr] > a[i]) inds.insert(i);
    }

    if (inds.count(i))
    {
      auto cur = inds.upper_bound(i);
      while (cur != inds.end() and a[*cur] >= a[i]) 
      {
        inds.erase(*cur);
        cur = inds.upper_bound(i);
      }
    }
