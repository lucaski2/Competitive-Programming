...
void solve(int tc)
{
  ...
  while (q--)
  {
    ...
    while (true)
    {
      ...
      for (int i = 0; i < n; i++)
      {
        if (start and p[i] < pos) continue;
        if (!start and !dir and p[i] <= pos) continue;
        if (dir and p[i] >= pos) continue;
        if ((abs(p[i] - pos) + off) % k == d[i])
        {
          if (ind == -1 and !dir) ind = i;
          if (dir) ind = i;
        }
      }
      start = false;
