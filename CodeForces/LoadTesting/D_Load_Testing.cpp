...
void solve(int tc)
{
  ...
  // amount to make current prefix equal to x, and what the value is

  vector<pair<int, int>> pref(n);
  pref[0].s = a[0];
  for (int i = 1; i < n; i++)
  {
    int incr = max(0ll, pref[i - 1].s - a[i] + 1);
    pref[i].f = pref[i - 1].f + incr;
    pref[i].s = max(a[i], pref[i - 1].s + 1);
  }
  ...
  for (int i = n - 2; i >= 0; i--)
  {
    int incr = max(0ll, suff[i + 1].s - a[i] + 1);
    suff[i].f = suff[i + 1].f + incr;
    suff[i].s = max(a[i], suff[i + 1].s + 1);
  }
  int ans = INT64_MAX;
  for (int i = 0; i < n; i++)
  {
    int am1 = (i == 0 ? 0 : pref[i - 1].f);
    int am2 = (i == n - 1 ? 0 : suff[i + 1].f);
    int am3 = (i == 0 ? 0 : pref[i - 1].s);
    int am4 = (i == n - 1 ? 0 : suff[i + 1].s);
    ans = min(ans, am1 + am2 + max(0ll, max(am3, am4) - a[i] + 1));
  }
  cout << ans << endl;
