...
void solve(int tc)
{
  ...
  for (int i = 0; i < q; i++)
  {
    if (que[i].t)
    {
      seggy.update(start[que[i].v], last[que[i].v], que[i].x);
    }
    else
    {
      cnt++;
      seggy.update(start[cnt], start[cnt], -seggy.query(start[cnt]));
    }
    // for (int j = 0; j < n; j++)
    // {
    //   cout << seggy.query(start[j]) << ' ';
    // }
    // cout << endl;
  }
  // cout << cnt << endl;
