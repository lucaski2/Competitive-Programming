...
void solve(int tc)
{
  ...
  function<void(int)> dfs = [&](int cur)
  {
    ...
    b[cur] = (ans != -1 ? ans - a[cur] : 1e15);
  };
  int ans = 0;

  auto dfs2 = [&](auto self, int cur, int depth) -> map<int, int>
  {
    map<int, int> ret;
    if (b[cur] > 0) 
    {
    ...
    for (int neighbor : graph[cur])
    {
      map<int, int> oth = self(self, neighbor, depth + 1);
      for (auto ele : oth)
      ret[ele.f] += ele.s;
    }
    while (b[cur] < 0)
    {
      ...
      if (ele.s > -b[cur])
      {
        ans += (ele.f - depth) * -b[cur];
        ret[ele.f] += b[cur];
      }
      else
      {
        ans += (ele.f - depth) * ele.s;
        ret.erase(ele.f);
      }
