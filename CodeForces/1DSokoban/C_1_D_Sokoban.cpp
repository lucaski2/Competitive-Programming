...
int solve(vector<int> a, vector<int> b)
{
  ...
  for (int i = 0; i < m; i++)
  {
    auto ptr = lower_bound(all(a), b[i]);
    int cnt = ptr - a.begin();
    ...
    {
      int mid = (l + r) / 2;
      // that means that the midth person should be < mid from here
      int x = a[mid - 1];
      if (x - b[i] >= mid)
      {
        r = mid - 1;
    ...
    ptr = upper_bound(all(inds), b[i] + best);

    int am = inds.end() - ptr;
    // cout << lower_bound(all(b), b[i] + best) - lower_bound(all(b), b[i]) << ' ' << best << ' ';

    am += lower_bound(all(b), b[i] + best) - lower_bound(all(b), b[i]);
    ret = max(ret, am);
  }
  // cout << endl;
