...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    while (right < n and cur <= k)
    {
      if (amounts[a[right]] == 0 and cur == k) break;
      if (amounts[a[right]] == 0) cur++;
      amounts[a[right]]++;
      right++;
    }

    if (right - i > best.second - best.first) 
    {
      best = {i, right};
    }
    amounts[a[i]]--;
    if (amounts[a[i]] == 0) cur--;
  }
