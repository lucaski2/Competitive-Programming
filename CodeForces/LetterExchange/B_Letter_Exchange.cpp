...
void solve(int tc)
{
  ...
  vector<vector<vector<int>>> graph(3, vector<vector<int>>(3));
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    sort(all(s));
    if (s == "inw") continue;
    if (count(all(s), s[0]) == 3)
    {
      vector<int> a = {0, 1, 2};
      a.erase(a.begin() + f(s[0]));
      for (int b : a)
      {
        graph[f(s[0])][b].push_back(i);
      }
    }
    else
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        if (count(all(s), g(j)) == 0) am1 = j;
        if (count(all(s), g(j)) == 2) am2 = j;
      }

      graph[am2][am1].push_back(i);
    }
  }
  struct info
  {
  ...
  for (int i = 0; i < 3; i++)
  {
    for (int j = 0; j < 3; j++)
    { 
      while (graph[i][j].size() and graph[j][i].size())
      {
        ans.push_back({graph[i][j].back(), g(i), graph[j][i].back(), g(j)});
        graph[i][j].pop_back();
        graph[j][i].pop_back();
      }
    }
  ...
  for (int i = 0; i < 3; i++)
  {
    for (int j = 0; j < 3; j++)
    {
      if (graph[i][j].size())
      {
        for (int k = 0; k < 3; k++)
        {
          if (graph[j][k].size() and graph[k][i].size())
          {
            info c = {graph[i][j].back(), g(i), graph[j][k].back(), g(j)};
            info d = {graph[j][k].back(), g(i), graph[k][i].back(), g(k)};
            ans.push_back(c);
            ans.push_back(d);
            graph[i][j].pop_back();
            graph[j][k].pop_back();
  ...
  for (info ele : ans)
    cout << ele.i + 1 << ' ' << ele.b << ' ' << ele.j + 1 << ' ' << ele.c << endl;
