...
void solve(int tc)
{
  ...
  vector<int> psum(inds.size() + 1);
  for (int i = 0; i < inds.size(); i++)
    psum[i + 1] = psum[i] + inds[i];
  ...
  for (int p : prime)
  {
    ...
    vector<int> ele;
    for (int i = p; i <= n; i += p) if (n % i == 0) ele.push_back(i);
    ...
    for (int i = 0; i < n; i += p)
    {
      for (int s : ele)
      {
        ...
        if (j > n) break;
        int mid = (j + i) / 2;
        int am = (psum[j] - psum[mid]) - (j - mid) * inds[mid];
        int am2 = (mid - i) * inds[mid] - (psum[mid] - psum[i]);
        dp[j] = min(dp[j], dp[i] + am + am2);
      }
    }
