...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    for (int j = 1; j * j <= mx; j++)
    {
      ...
      if (j <= m) ptrs[j].push_back(a[i]);


      if (a[i] / j == j) continue;
      
      // also add a[i] / j
      if (a[i] / j <= m) ptrs[a[i] / j].push_back(a[i]);
    }
  }

  for (int i = 1; i <= m; i++)
  {
    // no ppl specialize in topic i
    if (ptrs[i].empty())
    {
      cout << -1 << endl;
      return;
  ...
  { 
    while (st.begin()->f < i)
    {
      ...
      st.insert({ptrs[cur.s].back(), cur.s});
      ptrs[cur.s].pop_back();
    }
