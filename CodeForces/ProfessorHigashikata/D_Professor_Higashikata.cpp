...
void solve(int tc)
{
  ...
  // get ts number of 1s
  // how to get the dih in the prefix
  // maybe us ds pmo
  ...
  // what is the order each element appears
  // ts is all disjoint anyway (one set) 
  ...
  for (int i = 0; i < m; i++)
  {
    ...
    int ind = inter[i].f;
    pair<int, int> bound = {ind, ind};
    auto ptr = left.upper_bound({ind, INT_MAX});

    if (ptr != left.begin())
    {
      ptr--;
      pair<int, int> cur = *ptr;
      if (cur.s >= ind)
      {
        left.erase(cur);
        bound.f = cur.f;
    ...
    while (ind <= inter[i].s)
    {
      ptr = left.lower_bound({ind, -1});
      if (ptr == left.end() or ptr->f > ind)
      {
        bound.s = ind;
        order.push_back(ind++);
      }
  ...
  for (int i = 0; i < n; i++)
  {
    if (inds.count(i) == 0) continue;
    seggy.update(inds[i], t[i]);
  }
  

  int cnt = count(all(t), 1);
  // if (m > 1e3 and n == 2)
  // {
  //   for (int ord : order)
  //     cout << ord << ' ';
  ...
  while (q--)
  {
    ...
    c--;
    t[c] = 1 - t[c];
    if (inds.count(c)) seggy.update(inds[c], t[c]);
    if (t[c]) cnt++;
    else cnt--;
    // cout << cnt << endl;
    ...
    if (cnt <= order.size())
    {
      cout << cnt - seggy.query(0, cnt - 1) << endl;
    }
    else
    {
      cout << order.size() - seggy.query(0, order.size() - 1) << endl;
    }
