...
void solve(int tc)
{
  ...
  {
    cin >> t[i].f >> t[i].s;
    pair<double, int> cur = {t[i].s / (double)t[i].f, i};
    if (t[i].f >= w) best = max(best, cur);
  }
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    for (int j = 0; j < mx; j++)
    {
      if (j != 0 and dp[j] == 0) continue;
      next[j + a[i].f] = max(next[j + a[i].f], dp[j] + a[i].s);
  ...
  int s = 0;
  for (int i = w; i < mx; i++)
    ans = max(ans, (int)(dp[i] / (double)(i) * 1000.0));
  if (best.f != INT_MAX)
  {
    pair<int, int> am = {t[best.s].f, t[best.s].s};

    sort(all(a), [](pair<int, int> a, pair<int, int> b) {return ((double)a.s / a.f > (double)b.s / b.f); });

    for (int i = 0; i < n; i++)
    {
      double rat = a[i].s / (double)a[i].f;
      double rat2 = am.s / am.f;
      if (rat > rat2)
      {
        am.f += a[i].f;
        am.s += a[i].s;
