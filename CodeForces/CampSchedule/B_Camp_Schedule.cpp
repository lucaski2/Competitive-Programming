...
void solve(int tc)
{
  ...
  while (p[m - 1]--) t.pop_back();
  while (*min_element(all(cnts)) >= 0)
  {
    for (int i = 0; i < t.size(); i++)
    {
      cnts[t[i] - '0']--;
    }
    if (*min_element(all(cnts)) >= 0) cout << t;
  }
  for (int i = 0; i < t.size(); i++)
  {
    cnts[t[i] - '0']++;
  }
  while (cnts[0]-- > 0) cout << '0';
  while (cnts[1]-- > 0) cout << '1';
