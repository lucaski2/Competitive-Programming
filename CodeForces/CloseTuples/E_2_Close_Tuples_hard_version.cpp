...
int choose(int n, int k) { if (k > n) return 0; return (factorial(n) * pow(factorial(k), mod - 2, mod)) % mod * pow(factorial(n - k), mod - 2, mod) % mod; }
...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    int ind = lower_bound(all(a), a[i] - k) - a.begin();
    int am = i - ind + 1;
    ans += choose(am - 1, m - 1);
    ans %= mod;
  }
