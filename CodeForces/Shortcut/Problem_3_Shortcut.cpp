...
void solve(int tc)
{
  ...
  while (m--)
  {
    ...
    cin >> d >> b >> c;
    d--; b--;
    graph[d].push_back({b, c});
    graph[b].push_back({d, c});
  }

  vector<pair<int, int>> dist(n, {1e12, 1e12});
  ...
  while (pq.size())
  {
    ...
    for (auto neigh : graph[cur.s])
    {
      pair<int, int> next = {neigh.s + cur.f, cur.s};
      if (next < dist[neigh.f])
      {
        dist[neigh.f] = next;
        pq.push({next.f, neigh.f});
      }
    }
  }
  ...
  for (int i = 0; i < n; i++)
  {
    if (dist[i].s >= 0) tree[dist[i].s].push_back(i);
  }
  int ans = 0;
