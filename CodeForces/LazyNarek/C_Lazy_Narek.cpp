...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    for (int j = 0; j < m; j++)
    {
      char c;
      cin >> c;
      if (count(all(order), c) == 0) continue;
      s.push_back(c);
    }
    if (s.size()) a.push_back(s);
  }
  n = a.size();
  // the shortest string length that gets up to char x
  vector<pair<int, int>> dp(5, {INT_MAX, 0});
  dp[0].f = 0;
  for (int i = 0; i < n; i++)
  {
    // try to propogate each of ur nareks
    vector<pair<int, int>> next = dp;
    ...
    for (int ele = 0; ele < 5; ele++)
    {
      ...
      // best = 5 * dp[i].s * 2 - dp[i].f
      int best = 5 * cnt * 2 - l;
      if (best > 5 * next[cur].s * 2 - next[cur].f)
      {
        next[cur] = {l, cnt};
      }
    }
