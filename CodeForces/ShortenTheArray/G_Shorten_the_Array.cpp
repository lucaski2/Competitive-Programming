...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    for (int j = m - 1; j >= 0; j--)
    {
      // the side that gives one
      node* one = (((1 << j) & a[i]) > 0 ? cur->l : cur->r);
      node* zero = (((1 << j) & a[i]) > 0 ? cur->r : cur->l);
      ...
      {
        // binary search on pts
        if (one)
        {
          auto lo = upper_bound(all(one->pts), i);
          if (lo != one->pts.begin())
          {
            lo--;
            ans = min(ans, i - *lo);
    ...
    if (cur)
    {
      auto lo = upper_bound(all(cur->pts), i);
      if (lo != cur->pts.begin())
      {
        lo--;
        ans = min(ans, i - *lo);
