...
void solve(int tc)
{
  ...
  // 3 cases
  // case 1, there is only one dangerous element (ans += n) gets handled in case 3
  // case 2, one element is an ancestor of the other
  // case 3, none are ancestors
  dfs(0, -1);
  for (int ele : dp)
    ans += ele;
  ans %= mod;
  cout << (ans + 1) % mod << endl;
  // cout << (dfs(0, -1) + ans) % mod << endl;
