...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    auto r = upper_bound(all(a), a[i] + k);
    save[i] = r - l;
  }

  vector<int> saveMax(n);
  saveMax[n - 1] = save[n - 1];
  for (int i = n - 2; i >= 0; i--)
    saveMax[i] = max(saveMax[i + 1], save[i]);

  int ans = 0;
  for (int i = 0; i < n; i++)
  {
    int am = save[i];
    auto ptr = upper_bound(all(a), a[i] + k);
    if (ptr != a.end())
    {
      am += saveMax[ptr - a.begin()];
    }
    ans = max(ans, am);
  }
  cout << ans << endl;
