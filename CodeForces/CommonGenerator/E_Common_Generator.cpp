...
void solve(int tc)
{
  ...
  // after making the lowest prime factor, the number should still be greater than the prime
  int p = *pele.begin();
  for (int i = 0; i < n; i++)
  {
    int lo = *min_element(all(divs[a[i]]));
    if (a[i] == p) continue;
    
    // if integer is even, then it works if its >= p * 2
    if (a[i] % 2 == 0 and p * 2 > a[i])
    {
      cout << -1 << endl;
      return;
    }
