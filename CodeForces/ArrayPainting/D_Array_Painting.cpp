...
void solve(int tc)
{
  ...
  // if consecutive blocks: 
  // if there is a 2 in there then u get both sides
  // otherwise u only get 1
  // always choose the earliest one unless already taken
  int start = 0;
  int mx = 0;
  for (int i = 0; i < n; i++)
  {
    ...
    if (a[i] == 0 or i == n - 1)
    {
      ...
      if (mx == 2)
      {
        if (start > 0) color[start - 1] = true;
        color[i] = true;
      }
      if (mx == 1)
      {
        if (start > 0 and !color[start - 1]) color[start - 1] = true;
        else color[i] = true;
      }
  ...
  cout << ans + n - accumulate(all(color), 0) << endl;
