...
void solve(int tc)
{
  ...
  // if there are n directions you can get to block x from
  // then block x can be used if the degree of block x is <= n + 1
  ...
  for (int i = 0; i < n; i++)
  {
    for (int j = 0; j < m; j++)
    {
      // cout << deg[i][j] << ' ';
      if (a[i][j] != '.') continue;
      if (deg[i][j] <= 1 and vis[i][j]) a[i][j] = '+';
    }
    // cout << endl;
  }
  // cout << endl;
