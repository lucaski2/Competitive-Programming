...
void solve(int tc)
{
  ...
  while (pq.size())
  {
    ...
    pq.pop();
    if (dist[cur.f] < cur.s) continue;

    for (pair<int, int> neighbor : graph[cur.f])
    {
      int next = cur.s + neighbor.s;
      if (next >= dist[neighbor.f]) continue;
      last[neighbor.f] = cur.f;
      dist[neighbor.f] = next;
      pq.push({neighbor.f, next});
    }
  }
  vector<int> ans;
  int cur = n - 1;
