...
void solve(int tc)
{
  ...
  for (char ch : things)
  {
    if (count(all(s), ch) > 0) incl.push_back(ch);
  }

  if (incl.size() == 2 and incl != "LR" and incl != "UD")
  {
    int cnt = 0;
    if (incl[0] == 'L' and dx >= 0) cnt++;
    if (incl[0] == 'R' and dx <= 0) cnt++;
    if (incl[1] == 'U' and dy <= 0) cnt++;
    if (incl[1] == 'D' and dy >= 0) cnt++;
    if (cnt >= 2)
    {
      cout << -1 << endl;
      return;
  ...
  if (incl.size() == 1)
  {
    ...
    if (incl[0] == 'L' and dx >= 0) cnt++;
    if (incl[0] == 'R' and dx <= 0) cnt++;
    if (incl[0] == 'U' and dy <= 0) cnt++;
    if (incl[0] == 'D' and dy >= 0) cnt++;
    if (cnt >= 1)
    {
      cout << -1 << endl;
      return;
