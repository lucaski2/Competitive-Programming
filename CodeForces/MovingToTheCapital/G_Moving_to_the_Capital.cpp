...
void solve(int tc)
{
  ...
  while (pq.size())
  {
    ...
    for (int neighbor : rev[cur.s])
    {
      if (depth[neighbor] >= depth[cur.s])
      {
        if (depth[cur.s] < dp[neighbor])
        {
          dp[neighbor] = depth[cur.s];
          pq.push({depth[cur.s], neighbor});
        }
      }
      else
      {
        if (dp[cur.s] < dp[neighbor])
        {
          dp[neighbor] = dp[cur.s];
