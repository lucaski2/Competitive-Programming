...
void solve(int tc)
{
  ...
  vector<vector<int>> mins(n, vector<int>(m + 1, INT64_MAX));
  vector<int> ands(1 << m);
  vector<int> ppcnts(1 << m);
  for (int i = 1; i < (1 << m); i++)
  {
    int cur = (1 << 32) - 1;
    for (int j = 0; j < m; j++)
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    {
      int am = ands[j] & a[i];
      mins[i][ppcnts[j]] = min(mins[i][ppcnts[j]], am);
    }
  }
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    for (int j = 1; j < m + 1; j++)
    {
      diffs.push_back(last - mins[i][j]);
      // cout << mins[i][j] << ' ' << last << endl;
  ...
  for (int i = 0; i < diffs.size() and i < k; i++)
    ans -= diffs[i];
  cout << ans << endl;
