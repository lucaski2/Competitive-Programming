...
void precomp()
{
  ...
  for (int ele : s)
    if ((int)(sqrtl(ele)) * (int)(sqrtl(ele)) != ele) bs.push_back(ele);
}
...
void solve(int tc)
{
  ...
  int ans = (int)(sqrtl(hi)) - (int)(sqrtl(lo - 1));
  int ind1 = upper_bound(all(bs), hi) - bs.begin();
  int ind2 = lower_bound(all(bs), lo) - bs.begin();
  ans += ind1 - ind2;
  cout << ans << endl;
}
