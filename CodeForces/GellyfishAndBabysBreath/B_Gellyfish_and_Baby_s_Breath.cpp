...
void solve(int tc)
{
  // permutations, only have to check ones with max
  int n;
  cin >> n;
  vector<int> p(n), q(n);
  for (int i = 0; i < n; i++)
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    int am1 = pow(2, p[mp], mod) + pow(2, q[i - mp], mod);
    int am2 = pow(2, q[mq], mod) + pow(2, p[i - mq], mod);
    if (p[mp] > q[mq] or (p[mp] == q[mq] and q[i - mp] >= p[i - mq]))
    {
      cout << am1 % mod << ' ';
    }
    else if (q[mq] > p[mp] or (p[mp] == q[mq] and q[i - mp] <= p[i - mq])) cout << am2 % mod << ' ';
  }
  cout << endl;
