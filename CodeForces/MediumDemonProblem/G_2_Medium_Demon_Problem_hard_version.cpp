...
void solve(int tc)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    {
      visited[cur] = true;
      for (int neighbor : graph[cur]) if (!visited[neighbor]) dfs(neighbor);
    };
    dfs(i);
    ...
    // figure out how to calc for the tree for each element in the cycle
    
    for (int ele : cycle)
    {
      for (int neighbor : rev[ele])
      {
        if (cycle.find(neighbor) != cycle.end()) continue;
        int cnt = 0;
