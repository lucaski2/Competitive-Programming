...
void solve(int tc)
{
  ...
  for (int i = 0; i < n - 1; i++)
  {
    ...
    edges[{min(b, c), max(b, c)}] = d;
    ans += d;
  }

  vector<unordered_map<int, int>> children(n);
  vector<int> schild(n);
  vector<int> par(n, -1);
  ...
  function<void(int, int)> dfs = [&](int cur, int p)
  {
    ...
    {
      if (neighbor.f == p) continue;
      dfs(neighbor.f, cur);
      children[cur][a[neighbor.f]] += neighbor.s;
    }
    schild[cur] = children[cur][a[cur]];
    ans -= schild[cur];
  };
  ...
  while (q--)
  {
    ...
    a[c] = d;
    // recalculate node
    int next = children[c][a[c]];
    ans -= next - schild[c];
    schild[c] = next;
    if (par[c] != -1)
    {
      int parent = par[c];
      children[parent][old] -= edges[{min(parent, c), max(parent, c)}];
      children[parent][a[c]] += edges[{min(parent, c), max(parent, c)}];
      next = children[parent][a[parent]];
      ans -= next - schild[parent];
      schild[parent] = next;
    }
