...
int main()
{
    ...
    for (int i = 1; i <= k; i++)
    {
        ...
        for (int ind : m[i])
        {
            segs.push_back({max(0, ind - x), ind + 1});
        }
        int ans = psum[n];
        // compress segments going from back of diff_segs
        vector<pair<int, int>> compressed_segs;
        
        for (int i = segs.size() - 1; i >= 0; i--)
        {
            if (compressed_segs.empty())
            {
                compressed_segs.push_back(segs[i]);
            }
            else
            {
                if (segs[i].second >= compressed_segs.back().first)
                {
                    compressed_segs.back().first -= x + 1;
                }
                else
                {
                    compressed_segs.push_back(segs[i]);
        ...
        for (pair<int, int> seg : compressed_segs)
        {
            ans -= psum[max(0, seg.second)] - psum[max(0, seg.first)];
        }
