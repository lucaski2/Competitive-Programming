...
void solve(int tc)
{
  ...
  Seggy<pair<int, int>> left;
  Seggy<pair<int, int>> right;
  left.init(n, {INT_MAX, INT_MAX});
  right.init(n, {INT_MAX, INT_MAX});

  for (int i = 0; i < n; i++)
  {
    left.update(i, {a[i] + n - i - 1, i});
    right.update(i, {a[i] + i, i});
  }
  ...
  for (int i = 0; i < q; i++)
  {
    ...
    if (c == 1)
    {
      ...
      cin >> k >> x;
      k--;
      left.update(k, {x + n - k - 1, k});
      right.update(k, {x + k, k});
      a[k] = x;
    }
    else
    {
      ...
      pair<int, int> l = left.query(0, k);
      pair<int, int> r = right.query(k, n - 1);
      int am1 = a[l.s] + (k - l.s);
      int am2 = a[r.s] + (r.s - k);
      cout << min(am1, am2) << en;
    }
