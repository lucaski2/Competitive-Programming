...
unique_ptr<ost<pair<int, int>>> dfs(int cur, int par, int depth)
{
  auto depths = make_unique<ost<pair<int, int>>>();
  depths->insert({depth, cur});
  
  for (int neighbor : graph[cur])
  {
    if (neighbor == par) continue;

    auto ret = dfs(neighbor, cur, depth + 1);

    if (ret->size() > depths->size()) 
      swap(ret, depths); // now `depths` is the larger set

    for (auto ele : *ret)
    {
      int needed = depth + (k1 - (ele.first - depth));
      int needed2 = depth + (k2 - (ele.first - depth));
      ans += depths->order_of_key({needed2, INT_MAX}) - depths->order_of_key({needed, -1});
    }
    ...
    // ret gets automatically deleted when out of scope
  }
