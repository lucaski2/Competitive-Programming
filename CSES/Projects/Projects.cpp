...
{ 
  ...
  priority_queue<int> best_rewards;
  priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> in_queue;
  vector<info> a(n);
  for (int i = 0; i < n; i++)
    cin >> a[i].l >> a[i].r >> a[i].c;
  best_rewards.push(0);
  sort(a.begin(), a.end(), info::comp);

  int ans = 0;
  for (int i = 0; i < n; i++)
  {
    while (!in_queue.empty() and in_queue.top().first < a[i].l)
    {
      best_rewards.push(in_queue.top().second);
      in_queue.pop();
    }
    int time = a[i].r;
    int am = a[i].c + best_rewards.top();
    ans = max(ans, am);
    in_queue.push({time, am});
  }
  cout << ans << endl;
