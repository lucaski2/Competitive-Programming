...
void solve(int tc)
{
  ...
  multiset<int, greater<int>> inds;
  
  for (int i = 0; i < n; i++)
  {
    while (!pq.empty() and pq.top().first > a[i].first)
    {
      pair<int, int> cur = pq.top();
      pq.pop();

      inds.insert(cur.second);
    }
    ...
    {
      auto it = inds.lower_bound(a[i].second);
      if (it == inds.end())
      {
        pq.push({a[i].first, a[i].second});
        ans++;
