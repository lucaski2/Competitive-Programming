...
void solve(int tc)
{
  ...
  function<void(int, int, int)> dfs = [&](int cur, int depth, int parent)
  {
    start[cur] = time;
    depths[cur] = depth;
    et.push_back(depth);
  ...
  while (q--)
  {
    ...
    int lca = sparse.query(min(start[a], start[b]), max(start[a], start[b]));
    cout << depths[a] + depths[b] - lca * 2 << endl;
  }
}
