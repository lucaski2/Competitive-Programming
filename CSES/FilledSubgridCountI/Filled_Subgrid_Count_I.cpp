...
void solve(int tc)
{
  ...
  vector<vector<vector<bool>>> grids(k, vector<vector<bool>>(n, vector<bool>(n)));
  vector<vector<vector<int>>> psums(k, vector<vector<int>>(n + 1, vector<int>(n + 1)));
  ...
  for (int i = 0; i < k; i++)
  {
    for (int j = 1; j <= n; j++)
    {
      for (int l = 1; l <= n; l++)
      {
        psums[i][j][l] = grids[i][j - 1][l - 1] + psums[i][j - 1][l] + psums[i][j][l - 1] - psums[i][j - 1][l - 1];
      }
    }
  }
  vector<int> ans(k);
  for (int i = 0; i < n; i++)
  {
    for (int j = 0; j < n; j++)
    {
      ...
      {
        int mid = (l + r) / 2;
        int am = psums[ind][i + 1][j + 1] - psums[ind][i + 1][j - mid] - psums[ind][i - mid][j + 1] + psums[ind][i - mid][j - mid];
