...
void solve(int tc)
{
  ...
  {
    if (x < 0 or x >= 7 or y < 0 or y > 6) return;
    ...
    if (x == 0 and y == 6) return;
    visited[x][y] = true;


    if (s[i] == '?') {
      vector<pair<int, int>> diffs = {{0, 1}, {1, 0}, {-1, 0}, {0, -1}};

      for (pair<int, int> diff : diffs)
      {
        dfs(diff.f + x, diff.s + y, i + 1);
      }
    }
    else
    {
      if (s[i] == 'L') dfs(x - 1, y, i + 1);
      if (s[i] == 'R') dfs(x + 1, y, i + 1);
      if (s[i] == 'D') dfs(x, y + 1, i + 1);
      if (s[i] == 'U') dfs(x, y - 1, i + 1);
    }
