...
void solve(int tc)
{
  ...
  for (int i = 0; i < n * 2 - 2; i++)
  {
    ...
    {
      if (ele.f < n - 1) lo = min(lo, grid[ele.f + 1][ele.s]);
      if (ele.s < n - 1) lo = min(lo, grid[ele.f][ele.s + 1]);
    }
    ans.push_back(lo);

    for (pair<int, int> ele : cur)
    {
      if (ele.f < n - 1 and grid[ele.f + 1][ele.s] == lo and !visited[ele.f + 1][ele.s])
      {
        visited[ele.f + 1][ele.s] = true;
        next.push_back({ele.f + 1, ele.s});
      }

      if (ele.s < n - 1 and grid[ele.f][ele.s + 1] == lo and !visited[ele.f][ele.s + 1])
      {
        visited[ele.f][ele.s + 1] = true;
        next.push_back({ele.f, ele.s + 1});
      }
    }
    cur = next;
