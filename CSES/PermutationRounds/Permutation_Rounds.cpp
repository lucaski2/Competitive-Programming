...
void solve(int tc)
{
  ...
  int mx = 2e5+5;
  vector<vector<pair<int, int>>> seive(mx);

  for (int i = 2; i < mx; i++)
  {
    if (seive[i].size()) continue;
  ...
  for (int ele : cycles)
  {
    for (pair<int, int> am : seive[ele])
      counts[am.f] = max(am.s, counts[am.f]);
  }
  int ans = 1;
  for (int i = 1; i < mx; i++)
  {
    ans = (ans * pow(i, counts[i], mod)) % mod;
    // cout << (ans * pow(i, counts[i], mod)) << endl;
  }
  cout << ans << endl;
