...
void solve(int tc)
{
  ...
  set<pair<int, int>> dudes;
  vector<char> chars(n);
  pair<int, int> cur = {0, 0};
  dudes.insert(cur);
  pair<int, int> dude = {r, c};
  for (int i = 0; i < n; i++)
  {
    char d;
    cin >> d;
    ...
    if (d == 'N') cur.first++;
    else if (d == 'W') cur.second++;
    else if (d == 'S') cur.first--;
    else cur.second--;
    if (d == 'N') dude.first++;
    else if (d == 'W') dude.second++;
    else if (d == 'S') dude.first--;
    else dude.second--;
    dudes.insert(cur);

    if (dudes.find(dude) != dudes.end()) cout << '1';
    else cout << '0';
  }
  cout << endl;
}
