...
void solve(int tc)
{
  ...
  while (q--)
  {
    ...
    if (t == 1)
    {
      ...
      cin >> i >> x;
      i--;
      // please be this case all the fucking time
      if (x == s[i]) continue;
      if (i > 0 and x == s[i - 1])
      {
    ...
    else
    {
      ...
      cin >> l >> r;
      l--; r--;
      // first, find lowest lower bound under l
      auto ptr = left.lower_bound({l, -1});
      int ans = 0;
      if (ptr != left.begin())
      {
        ptr--;
        ans = max(ans, ptr->s - l + 1);
        l = max(l, ptr->s + 1);
      }
      ptr = right.upper_bound({r, -1});
      ans = max(ans, r - ptr->s + 1);
      r = min(r, ptr->s - 1);
      
      cout << (l <= r ? max(seggy.query(l, r), ans) : ans) << endl;
    }
  }
