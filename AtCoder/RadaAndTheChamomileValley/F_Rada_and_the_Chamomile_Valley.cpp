...
// we need to get depths
// dfs and find any edge
...
void solve(int tc)
{
  ...
  function<void(int, int, int)> tarjan = [&](int cur, int par, int depth)
  {
    visited[cur] = true;
    depths[cur] = depth;
    lo[cur] = depth;
    ...
    { 
      ...
      if (visited[neighbor])
      {
        lo[cur] = min(lo[cur], depth[neighbor]);
      }
      else
      {
        tarjan(neighbor, cur, depth + 1);
        lo[cur] = min(lo[cur], lo[neighbor]);
        if (lo[neighbor] > depths[cur])
          bridges.insert(ind);
      }
    }
  };
