...
void solve(int tc)
{
  ...
  {
    cont[a[i]]++;
    if (not_cont.find(a[i]) != not_cont.end()) not_cont.erase(a[i]);
  }
  int ans = *not_cont.begin();
  ...
  for (int i = m; i < n; i++)
  {
    ...
    cont[last]--;
    if (cont[last] == 0)
       not_cont.insert(last);

    cont[a[i]]++;
    if (not_cont.find(a[i]) != not_cont.end())
      not_cont.erase(a[i]);
    ans = min(ans, *not_cont.begin());
  } 
  cout << ans << endl;
