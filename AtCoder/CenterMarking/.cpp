...
void solve(int tc)
{
  ...
  string s;
  cin >> s;
  int am1 = count(all(s), '1');
  int am2 = count(all(s), '2');
  int am3 = count(all(s), '3');
  int am4 = count(all(s), '4');
  vector<int> ams = {am1, am2, am3, am4};
  cout << *max_element(all(ams)) << ' ' << *min_element(all(ams)) << endl;
