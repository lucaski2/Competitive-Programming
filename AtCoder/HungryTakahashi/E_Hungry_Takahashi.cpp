...
void solve(int tc)
{
  ...
  while (l <= r)
  {
    ...
    vector<vector<int>> dp(n, vector<int>(m, -2e14));
    dp[0][0] = mid;
    ...
    for (int i = 0; i < n; i++)
    {
      for (int j = 0; j < m; j++)
      {
        int am = dp[i][j] + grid[i][j] - a[i + j];
        if (am < 0) continue;
        if (i < n - 1)
        {
          dp[i + 1][j] = max(dp[i + 1][j], am);
        }
        if (j < m - 1) dp[i][j + 1] = max(dp[i][j + 1], am);
      }
    }
    if (dp[n - 1][m - 1] + grid[n - 1][m - 1] - a[n + m - 2] >= 0)
    {
      r = mid - 1;
      ans = mid;
    }
