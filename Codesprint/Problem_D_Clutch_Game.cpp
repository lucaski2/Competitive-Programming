...
void solve(int tc)
{
  // chance of getting to certain score difference

  int t, s;
  cin >> t >> s;

  // at each time, for each score, for each team, calculate probability
  // dp[i][j][k] i = time, j = score, k = team
  vector<vector<vector<double>>> dp(t + 1, vector<vector<double>>(t * 3 + 1, vector<double>(2)));
